...
One-line CSS change: `.band { height: 88vh → 100vh }` (`src/styles/portfolio.css:408`), matching the existing `.intro { height: 100vh }`. No JS change — the fixed nav (`.bar`) is an overlay (not in flow), and `landingStep()` scrolls to each target's `offsetTop` (`usePortfolioEngine.ts:273`), so taller bands land flush automatically. `min-height: 420px` and `.contact` left untouched. Verified: `npm run lint` + `npm run build` pass; pending owner eye-check of the landing.
